...
def solar(pixoo, hass, page_data, FONT_PICO_8, FONT_GICKO, FONT_THIN):
    for solar in page_data["PV"]:
        ...
        if rendered_discharge < 0: 
            pixoo.draw_text(f'{rendered_discharge}', (17, 21), red, FONT_THIN)
        else:
            pixoo.draw_text(f'{rendered_discharge}', (21, 21), green, FONT_THIN)
        ...
        if rendered_powerhousetotal < 0:
            pixoo.draw_text(f"{rendered_powerhousetotal}", (17, 37), blue, FONT_THIN)
        else:
            pixoo.draw_text(f"{rendered_powerhousetotal}", (21, 37), blue, FONT_THIN)
        

        pixoo.draw_image("/config/custom_components/divoom_pixoo/img/industry.png", (1, 48))
        if rendered_vomNetz < 0:
            pixoo.draw_text(f'{rendered_vomNetz}', (17, 53), grey, FONT_THIN)
        else:
            pixoo.draw_text(f'{rendered_vomNetz}', (21, 53), grey, FONT_THIN)
