...
def solar(pixoo, hass, page_data, FONT_PICO_8, FONT_GICKO):
    for solar in page_data["PV"]:
        ...
        if rendered_power >= 1:
            pixoo.draw_text(f'{rendered_power}', (18, 4), yellow, FONT_GICKO)
        else:
            pixoo.draw_text(f'{rendered_power}', (18, 4), grey, FONT_GICKO)
        ...
        pixoo.draw_text(f"{rendered_powerhousetotal}", (18, 36), blue, FONT_GICKO)

        pixoo.draw_image("/config/custom_components/divoom_pixoo/img/industry.png", (1, 48))
        pixoo.draw_text(f'{rendered_vomNetz}', (18, 52), grey, FONT_GICKO)
